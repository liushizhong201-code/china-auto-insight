(() => {
      ...
      return `
        <div class="bar-row">
          <div class="bar-meta">
            ...
            <span>${formatSignedWan(item.delta)}辆 · ${formatPercent(item.yoy_rate)}</span>
          </div>
  ...
  function renderPrice() {
    ...
    setText("price-callout-value", `${formatSignedWan(largestDecline.delta)}辆`);
    document.getElementById("price-list").innerHTML = groups.map((item) => `
      <div class="price-row">
        ...
        <span class="price-value">${formatSignedWan(item.delta)}辆</span>
      </div>`).join("");
  ...
  function renderQuality() {
    ...
    setText("quality-excluded", `${data.quality.excluded_series}个`);
